...
Maps are commonly used as choropleths, i.e. the polygon is colored based on a another variable. Thus, spatial data are often merged with other data.  Here, the USDA compiles some data by county in the US. The data are winnowed to identification variables and a single numeric named "unemployment_rate_2020". Then the USDA data is merged with the California counties data.
...
The order that the files are included in the merge matters. By putting the "ca" simple feature class dataframe first, the subsequent "usda" dataframe morphes into the same simple feature class and is now combined. See Professor Lovelace's Geospatial in R 3.2.4 [vector attribute joining](https://geocompr.robinlovelace.net/attr.html#vector-attribute-joining) for more details.
