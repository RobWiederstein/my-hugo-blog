...
date: '2022-10-19'
#slug: multinational-letter-frequency-analysis-on-works-of-literature
...
With the `Gutenbergr` package in R, public domain texts are freely available.   The collection is large and contains 60,000+ free eBooks in different languages.  I chose four well-known works of literature one from English (en), Spanish (es), German (de), and French (fr).  The texts are "Moby Dick" by Herman Melville, Don Quijote by Cervantes, Faust by Goethe and Les Miserables by Victor Hugo.  These were in the original language. Only the ten most frequent letters from Moby Dick were included. There were similar distributions of the letters for the other languages.


# [Letter Frequency](#letter-frequency)

```{r load-plot, fig.fullwidth=T, cache=FALSE}
data <- readRDS("./works_of_fiction.rds")
colors <- qualitative_hcl(4, palette = "dark2")
library(ggplot2)
data |>
  ggplot() +
  aes(pct, characters, group = language, color = language) +
  geom_point(size = 2) +
...
data <-
    data |>
    filter(characters %in% most_common_en_characters)
# reorder characters high --> low
data$characters <- factor(data$characters)
data$characters <- forcats::fct_reorder(data$characters, data$pct)
#reorder language
data$language <- factor(data$language)
data$language <- forcats::fct_relevel(
    data$language,
    "en", "es", "de", "fr",
    )
#reorder title
